...
DIM_OBS = 12 # no. of dimensions in observation space
...
class XarmEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # currentPose = p.getLinkState(self.xarmUid, 11)
        # currentPosition = currentPose[0]
        # newPosition = [currentPosition[0] + dx,
        #                currentPosition[1] + dy,
        #                currentPosition[2] + dz]
        # jointPoses = p.calculateInverseKinematics(self.xarmUid,11,newPosition, orientation)[0:7]

        # p.setJointMotorControlArray(self.xarmUid, list(range(7))+[9,10], p.POSITION_CONTROL, list(jointPoses)+2*[fingers])

        # METHOD 2: action = delta_q
        # Get the current joint angles
        joint_angles = [p.getJointState(self.xarmUid, i)[0] for i in range(7)]
        ...
        p.setJointMotorControlArray(self.xarmUid, list(range(7)), p.POSITION_CONTROL, joint_angles)
        
        p.stepSimulation()

        state_object, _ = p.getBasePositionAndOrientation(self.objectUid)
        state_robot = p.getLinkState(self.xarmUid, 6)
        state_robot = state_robot[0]+state_robot[2]+state_robot[4]
        #state_fingers = (p.getJointState(self.xarmUid,9)[0], p.getJointState(self.xarmUid, 10)[0])
        
        # panda arm tip state
        #tip_state = p.getLinkState(self.xarmUid, 10)[0] #p.getJointState(self.xarmUid, 10)
        ...
        # Detect Collisions of Panda and Object
        contacts = p.getContactPoints(self.xarmUid, self.objectUid)
        if contacts:
    ...
    def reset(self):
        ...
        # self.xarmUid = p.loadURDF("gym_panda/franka_panda/panda.urdf",useFixedBase=True,flags=p.URDF_USE_INERTIA_FROM_FILE)
        self.xarmUid = p.loadURDF(os.path.join(urdfRootPath, "xarm/xarm6_robot_white.urdf"),useFixedBase=True)

        for i in range(7):
            p.resetJointState(self.xarmUid,i, rest_poses[i])
        #p.resetJointState(self.xarmUid, 9, 0.08)
        #p.resetJointState(self.xarmUid,10, 0.08)
